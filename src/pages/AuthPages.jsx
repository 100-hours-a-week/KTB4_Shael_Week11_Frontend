...
export function LoginPage() {
  ...
  return <><header className="top-banner"><h1 className="banner-title">아무 말 대잔치</h1></header>
    <main className="login-page"><section className="login-container" aria-labelledby="loginTitle"><h2 id="loginTitle" className="login-title">로그인</h2>
...
export function SignupPage() {
  ...
  return <><header className="top-banner"><button type="button" className="back-button" onClick={() => navigate("/login/login.html")} aria-label="로그인 페이지로 이동">&lt;</button><h1 className="banner-title">아무 말 대잔치</h1></header>
    <main className="signup-page"><section className="signup-container"><h2 className="signup-title">회원가입</h2><form className="signup-form" noValidate onSubmit={submit}><div className="signup-box">
      <div className="profile-section"><p className="profile-label">프로필 사진</p><FieldError message={errors.profileImage} className="error-message profile-error" /><div className="profile-upload-wrapper"><button type="button" className="profile-upload-button" onClick={() => inputRef.current?.click()}><span className="profile-plus-icon">+</span>{preview && <img className="profile-preview" src={preview} alt="업로드한 프로필 이미지 미리보기" />}</button><input ref={inputRef} type="file" className="profile-file-input" accept="image/*" onChange={chooseFile} /></div></div>
      {[{ key: "email", label: "이메일", type: "email", placeholder: "이메일을 입력하세요", autoComplete: "email" }, { key: "password", label: "비밀번호", type: "password", placeholder: "비밀번호를 입력하세요", autoComplete: "new-password" }, { key: "confirm", label: "비밀번호 확인", type: "password", placeholder: "비밀번호를 한번 더 입력하세요", autoComplete: "new-password" }, { key: "nickname", label: "닉네임", type: "text", placeholder: "닉네임을 입력하세요", autoComplete: "nickname" }].map((field) => <div className="form-group" key={field.key}><label htmlFor={field.key} className="form-label">{field.label}</label><input id={field.key} type={field.type} required className="form-input" value={form[field.key]} onChange={(e) => update(field.key, e.target.value)} onBlur={() => blur(field.key)} placeholder={field.placeholder} autoComplete={field.autoComplete} /><FieldError message={errors[field.key]} /></div>)}
